...
const fetch = (options) => {
  ...
  for (let e in cloneData) {
    if (Object.prototype.hasOwnProperty.call(cloneData, e)) { // 避免获取从原型链继承的属性
      let v = cloneData[e]
      if (v instanceof moment) {
        v = v.format('YYYY-MM-DD HH:mm:ss').trim()
...
export default function request (options) {
  ...
  return fetch(options).then((response) => {
    ...
    if (response) {
      ...
      msg = (data.result && data.result.message) || statusText
    } else {
      status = 600
      msg = 'Network Error'
    }
    ...
    if (status === 401) {
      ...
      window.location = `${location.origin}/${prefix}/login?from=${from}`
    }
